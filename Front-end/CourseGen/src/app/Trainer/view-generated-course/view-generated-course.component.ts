...
export class ViewGeneratedCourseComponent {
  ...
  ngOnInit() {
    ...
    this.originalCourse = this.generateContentService.getGeneratedCourse();
    console.log('Original course data:', this.originalCourse);

    // === Use mock data for testing ===
    //this.originalCourse = dummyCourse;
    //console.log('Original course data (mock):', this.originalCourse);
    

    // Transform the course data for frontend usage
    this.generatedCourse = this.courseAdapter.adaptCourseForFrontend(this.originalCourse);
    console.log('Adapted course data:', this.generatedCourse);
    ...
    if (this.generatedCourse?.units) {
      this.parsedUnits = this.generatedCourse.units;
      
      this.units = this.parsedUnits.map(unit => UnitMapper.parsedUnitToUnit(unit));

      // Initialize expandedUnits object for each unit
      this.parsedUnits.forEach((unit: ParsedUnit) => {
        this.expandedUnits[unit.title] = false;
        // Pre-set loading state to false for each unit
        this.isLoading[unit.title] = false;
      });
    } else {
      console.error('No units found in adapted course data');
    }
  }
  ...
  get totalPages(): number {
    return Math.max(1, Math.ceil(this.parsedUnits.length / this.itemsPerPage));
  }
  ...
  async loadUnitContent(globalIndex: number): Promise<void> {
    try {
      const unit = this.parsedUnits[globalIndex];
      if (!unit?.content) {
        console.warn('No content found for unit at index', globalIndex);
        return;
      }
  
      // Mark the unit as loading
      this.isLoading[unit.title] = true;
  
      // Parse the content using the content parser service
      ...
      if (parsedContent && parsedContent.length > 0) {
        // Update the unit with parsed content
        this.parsedUnits[globalIndex] = {
          ...this.parsedUnits[globalIndex],
          ...parsedContent[0],
          isLoaded: true
        };
        console.log('Unit content loaded:', this.parsedUnits[globalIndex]);
      } else {
        console.warn('No parsed content returned for unit', unit.title);
        
        // Fallback: Extract content manually if parsing fails
        const introduction = this.extractTextBetweenHeadings(unit.content, "Introduction", "");
        const keyConcepts = this.extractBulletPoints(unit.content);
        
        this.parsedUnits[globalIndex] = {
          ...this.parsedUnits[globalIndex],
          introduction: introduction || unit.introduction || "No introduction available.",
          keyConcepts: keyConcepts.length ? keyConcepts : ["Content is being processed..."],
          sections: [
            {
              heading: "Content",
      ...
      // Handle the error by marking the unit as loaded anyway with an error message
      this.parsedUnits[globalIndex].isLoaded = true;
      this.parsedUnits[globalIndex].introduction = "Error loading content. Please try again.";
    } finally {
      // Make sure to always reset the loading state
      if (this.parsedUnits[globalIndex]) {
        this.isLoading[this.parsedUnits[globalIndex].title] = false;
      }
    }
  }
  
  private extractTextBetweenHeadings(content: string, heading: string, nextHeading: string): string {
  ...
  private extractBulletPoints(content: string): string[] {
    ...
    while ((match = bulletPointsRegex.exec(content)) !== null) {
  ...
  saveCourse() {
    this.generateContentService
      .saveGeneratedCourse(this.originalCourse) // Always save the original course data
      .subscribe({
        next: (response) => {
      ...
      next: (response) => {
        ...
        if (response.regeneratedContent) {
          ...
          if (unitIndex !== -1) {
            ...
            // Force reload of this unit
            this.parsedUnits[unitIndex].isLoaded = false;
            const title = this.parsedUnits[unitIndex].title;
            if (this.expandedUnits[title]) {
              // If the unit is currently expanded, reload its content
              this.loadUnitContent(unitIndex);
            }
          }
        }
